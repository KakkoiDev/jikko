...
func LoadAuth(root string) (*AuthFile, error) {
	...
	if !ok { return nil, fmt.Errorf("%s: no YAML frontmatter block found", path) }
	var a AuthFile
	if err := yaml.Unmarshal(front, &a); err != nil { return nil, fmt.Errorf("%s: %w", path, err) }
...
func writeAtomicMode(target string, data []byte, mode os.FileMode) error {
	if err := writeAtomic(target, data); err != nil { return err }
...
func ensureAuthGitignored(root string) error {
	...
	for _, line := range strings.Split(string(b), "\n") {
		if authIgnorePatterns[strings.TrimSpace(line)] { return nil }
	}
	prefix := string(b)
...
func (w *Workspace) CreateCredential(identity string) (string, error) {
	...
	if !ok { return "", fmt.Errorf("identity %q not found or ambiguous", identity) }
...
func (w *Workspace) Authenticate(token string) (*Page, error) {
	if token == "" { return nil, ErrAuthentication }
	a, err := LoadAuth(w.Root)
	if err != nil { return nil, err }
	want := []byte(tokenHash(token))
	matched := ""
	for _, c := range a.Credentials {
		if subtle.ConstantTimeCompare([]byte(c.TokenHash), want) == 1 { matched = c.Identity }
	}
	if matched == "" { return nil, ErrAuthentication }
	p, ok := w.ResolveIdentity(matched)
	if !ok { return nil, fmt.Errorf("%w: credential maps to unknown or ambiguous identity %q", ErrAuthentication, matched) }
	if len(p.Members) != 0 {
...
func (w *Workspace) RevokeCredentials(identity string) error {
	...
	for _, c := range a.Credentials {
		if c.Identity != stem { kept = append(kept, c) }
	}
	a.Credentials = kept
