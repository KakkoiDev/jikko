...
func TestUnusablePolicyDeniesEveryone(t *testing.T) {
	for name, frontmatter := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			p,_ := w.Resolve("s")
			if w.Allowed("alice", p, Read) { t.Fatal("unusable policy read as open") }
			if w.Allowed("nobody", p, Read) { t.Fatal("unusable policy read as open") }
			if err := w.ValidatePermissions(); err == nil { t.Fatal("unusable policy not reported") }
		})
	}
}
...
func TestIdentityCycleReported(t *testing.T) {
	...
	w, err := Open(d)
	if err != nil { t.Fatalf("a membership cycle must not fail the scan: %v", err) }
	if err := w.ValidateIdentities(); err == nil { t.Fatal("expected membership cycle to be reported") }
	// Membership resolution must still terminate.
	if w.MemberOf("a", "b") != true && w.MemberOf("a", "b") != false { t.Fatal("unreachable") }
}
...
func TestAmbiguousGrantReported(t *testing.T) {
	...
	writeTest(t,d,"secret.md","---\npermissions:\n  admin: admins\n---\n# Secret\n")
	w, err := Open(d); if err != nil { t.Fatal(err) }
	if err := w.ValidatePermissions(); err == nil { t.Fatal("ambiguous grant subject not reported") }
	p,_ := w.Resolve("secret")
	if w.Allowed("root", p, Admin) { t.Fatal("an unresolvable grant must not be honoured") }
}
...
func TestTokenAuthentication(t *testing.T) {
	...
	if info, err := os.Stat(filepath.Join(d,".auth.md")); err != nil { t.Fatal(err) } else if info.Mode().Perm() != 0600 { t.Fatalf("auth file mode = %v", info.Mode().Perm()) }
	p, err := w.Authenticate(token); if err != nil || p.Title != "Alice" { t.Fatalf("token did not authenticate: %v", err) }
	if _, err := w.Authenticate("wrong"); !errors.Is(err, ErrAuthentication) { t.Fatalf("wrong token: %v", err) }
	if _, err := w.Authenticate(""); !errors.Is(err, ErrAuthentication) { t.Fatalf("empty token: %v", err) }
...
func TestGitignoreNotDuplicated(t *testing.T) {
	...
	w, err := Open(d); if err != nil { t.Fatal(err) }
	if _, err := w.CreateCredential("alice"); err != nil { t.Fatal(err) }
	b, _ := os.ReadFile(filepath.Join(d,".gitignore"))
	if strings.Count(string(b), ".auth.md") != 1 { t.Fatalf("gitignore = %q", b) }
}
...
func TestMalformedAuthFileIsAnError(t *testing.T) {
	...
	w, err := Open(d); if err != nil { t.Fatal(err) }
	if _, err := LoadAuth(d); err == nil { t.Fatal("malformed auth file read as empty") }
...
func TestCapabilityStringIsTotal(t *testing.T) {
	if Admin.String() != "admin" || Read.String() != "read" { t.Fatal("known capabilities misnamed") }
	if Capability(99).String() == "" { t.Fatal("out-of-range capability must not panic or vanish") }
	if Capability(0).String() == "" { t.Fatal("zero capability must not panic or vanish") }
}
